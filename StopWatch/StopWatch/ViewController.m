...
- (void)setStopwatch:(LSIStopWatch *)stopwatch {
    ...
    if (stopwatch != _stopwatch) {
        ...
        // order correctly, open and close
        [self willChangeValueForKey:@"randomProp"]; // open nonexist
        [self willChangeValueForKey:@"stopWatch"];
        _stopwatch = stopwatch;
        [self didChangeValueForKey:@"stopWatch"];
        [self didChangeValueForKey:@"randomProp"]; // close nonexist
...
//    }
    ...
    if ([keyPath isEqualToString:@"running"]) {
        NSLog(@"Update the UI! Runing: %@", (self.stopwatch.running ? @"YES" : @"NO"));
        [self updateViews];
